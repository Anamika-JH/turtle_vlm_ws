...
class ChatInterface(tk.Tk):
    def __init__(self):
        ...
        self.llm_input_publisher = rospy.Publisher('/chat_input', String, queue_size=10)
        rospy.Subscriber('/chat_response', String, self.add_llm_response)
        rospy.Subscriber('/llm_image_output', Image, self.add_llm_image)
        self.bridge = CvBridge()
        self.title("ROS - Chat With Robot in Natural Language")
        self.geometry("550x650")
        self.configure(bg='white')
        script_dir = os.path.dirname(os.path.abspath(__file__))
        icon_path  = os.path.join(script_dir, "chat_logo.png")   # or .ico on Windows
        try:
            logo_icon = tk.PhotoImage(file=icon_path)
            self.iconphoto(False, logo_icon)        # sets the title-bar / task-bar icon
        except Exception as e:
            rospy.logwarn(f"Failed to set window icon: {e}")
    ...
    def add_llm_response(self, data):
        response = data.data
        self.after(0, self.add_message, "Turtle", response, "bot_msg", False)
